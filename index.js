...
let db = null;
let qrGeneratedAt = 0; // Track when QR was generated
...
app.get('/status', (req, res) => {
    const now = Date.now();
    const qrAge = qrGeneratedAt > 0 ? Math.round((now - qrGeneratedAt) / 1000) : 0;
...
// QR Code endpoint - returns nice HTML page with STABLE QR
app.get('/qr', (req, res) => {
...
// Reconnect page - clears QR and gets fresh one
app.get('/reconnect-page', async (req, res) => {
    qrCode = null;
    qrCodeBase64 = null;
    qrAttempts = 0;
...
// Keep the qr-image endpoint for backwards compatibility
...
app.get('/', (req, res) => {
    res.json({ 
        ...
        status: connectionStatus,
        connected_user: connectedUser?.name || null,
        qr_url: '/qr',
        status_url: '/status'
    });
});
